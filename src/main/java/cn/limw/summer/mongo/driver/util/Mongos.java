...
public class Mongos {
    ...
    public static Map<String, Object> getWhere(Expression expression, MongoPreparedStatement preparedStatement) {
        ...
        if (null == expression) {
            ...
            where = Maps.toMap(QueryOperators.AND, values);
            ...
            where = Maps.toMap(QueryOperators.OR, values);
            ...
            where = Maps.toMap(QueryOperators.IN, valueList.toArray());
        } else if (expression instanceof IsNullExpression) {
            Map<String, Object> map = Maps.toMap(((IsNullExpression) expression).isNot() ? QueryOperators.NIN : QueryOperators.IN, new Object[] { null });
            where = Maps.toMap(Parsers.getLeft(expression), map);
        } else {
            where = Maps.toMap(Parsers.getLeft(expression), Parsers.getRight(expression, preparedStatement));
        }
        return where;
    }
