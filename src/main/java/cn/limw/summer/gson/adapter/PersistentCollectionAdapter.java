 ...
 * @since Java7
 */
public class PersistentCollectionAdapter extends TypeAdapter<PersistentCollection> {
    protected static final PersistentCollectionAdapter INSTANCE = new PersistentCollectionAdapter();

    public static final TypeAdapterFactory FACTORY = new TypeAdapterFactory() {
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            return (PersistentCollection.class.isAssignableFrom(type.getRawType()) ? (TypeAdapter<T>) PersistentCollectionAdapter.INSTANCE : null);
        }
    };
