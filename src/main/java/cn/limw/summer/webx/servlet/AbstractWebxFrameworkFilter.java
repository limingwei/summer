package cn.limw.summer.webx.servlet;
...
import static com.alibaba.citrus.util.ServletUtil.getResourcePath;
...
/**
 * @author li
 * @version 1 (2015年6月9日 下午3:43:05)
 * @since Java7
 * @see com.alibaba.citrus.webx.servlet.WebxFrameworkFilter
 */
public class AbstractWebxFrameworkFilter extends FilterBean {
    ...
    protected final void init() throws Exception {
        ...
        if (parentContext instanceof WebxComponentsContext) {
            ...
            log.info("findParentContext() WebApplicationContext type is " + parentContext.getClass().getName());
        }

        WebxRootController rootController = Asserts.noNull(components, "components 为空").getWebxRootController();
        ...
        if (passthruFilter != null) {
            if (rootController instanceof PassThruSupportable) {
                ...
                log.warn(
                        "You have specified Passthru Filter in /WEB-INF/web.xml.  " + "It will not take effect because the implementation of WebxRootController ({}) does not support this feature.",
     ...
     * <li>假如指定了init-param <code>parentContextAttribute</code>，则查找指定的attribute key。假如没找到，则报错。</li>
     * </ul>
     */
