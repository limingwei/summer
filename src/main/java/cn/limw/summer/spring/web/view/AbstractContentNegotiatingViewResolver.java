...
/**
 * @author li
 * @version 1 (2014年11月11日 上午11:45:21)
 * @since Java7
 * @see org.springframework.web.servlet.view.ContentNegotiatingViewResolver
 */
public class AbstractContentNegotiatingViewResolver extends WebApplicationObjectSupport implements ViewResolver, Ordered, InitializingBean {
    protected static final Log logger = LogFactory.getLog(AbstractContentNegotiatingViewResolver.class);
     ...
     * <p>
     * If not set, ContentNegotiationManager's default constructor will be used, applying a {@link org.springframework.web.accept.HeaderContentNegotiationStrategy}.
     ...
     * <p>
     * For instance, when this flag is {@code true} (the default), a request for {@code /hotels.pdf} will result in an {@code AbstractPdfView} being resolved, while the {@code Accept} header can be the browser-defined {@code text/html,application/xhtml+xml}.
     ...
     * <p>
     * For instance, when this flag is {@code true}, a request for {@code /hotels?format=pdf} will result in an {@code AbstractPdfView} being resolved, while the {@code Accept} header can be the browser-defined {@code text/html,application/xhtml+xml}.
     * @deprecated use {@link #setContentNegotiationManager(ContentNegotiationManager)}
     */
    @Deprecated
     ...
     * <p>
     * If set to {@code true}, this view resolver will only refer to the file extension and/or parameter, as indicated by the {@link #setFavorPathExtension favorPathExtension} and {@link #setFavorParameter favorParameter} properties.
     * @deprecated use {@link #setContentNegotiationManager(ContentNegotiationManager)}
     */
    @Deprecated
     ...
     * <p>
     * This content type will be used when file extension, parameter, nor {@code Accept} header define a content-type, either through being disabled or empty.
     * @deprecated use {@link #setContentNegotiationManager(ContentNegotiationManager)}
     */
    @Deprecated
    ...
    /**
     * Indicate whether a {@link HttpServletResponse#SC_NOT_ACCEPTABLE 406 Not Acceptable} status code should be returned if no suitable view can be found.
    ...
    public View resolveViewName(String viewName, Locale locale) throws Exception {
        ...
        List<MediaType> requestedMediaTypes = getMediaTypes(((ServletRequestAttributes) requestAttributes).getRequest());
        if (requestedMediaTypes != null) {
            List<View> candidateViews = getCandidateViews(viewName, locale, requestedMediaTypes);
            View bestView = getBestView(candidateViews, requestedMediaTypes, requestAttributes);
            if (bestView != null) {
                return bestView;
            }
        }
    ...
    protected List<MediaType> getMediaTypes(HttpServletRequest request) {
        try {
            ...
            acceptableMediaTypes = acceptableMediaTypes.isEmpty() ? Collections.singletonList(MediaType.ALL) : acceptableMediaTypes;
     ...
     * 变 private 为 public
     */
    public View getBestView(List<View> candidateViews, List<MediaType> requestedMediaTypes, RequestAttributes attrs) {
