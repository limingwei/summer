 ...
 * @since Java7
 */
public class IsNullFilterTest extends BaseTest {
    @Resource
    ...
    public void test() {
        ...
        searchCriteria.setFilter("{'$is_null':['id','email']}");
        searchCriteria.setEntityName("User");
        searchQueryBuilder.doBuild(searchCriteria);
        Assert.assertEquals("FROM User entity WHERE entity.id IS NULL AND entity.email IS NULL", searchCriteria.getListQuery());
    }
}