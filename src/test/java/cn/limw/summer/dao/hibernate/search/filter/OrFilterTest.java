 ...
 * @since Java7
 */
public class OrFilterTest extends BaseTest {
    @Resource
    ...
    public void test() {
        ...
        searchCriteria.setFilter("{'$or':[{'$between':{'id':[2,1]}},{'$eq':{'id':2}}]}");
        searchCriteria.setEntityName("User");
        searchQueryBuilder.doBuild(searchCriteria);
        Assert.assertEquals("FROM User entity WHERE (entity.id BETWEEN :$1 AND :$2) OR (entity.id=:$3)", searchCriteria.getListQuery());
    }
}