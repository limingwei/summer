 ...
 * @since Java7
 */
public class LikeFilterTest extends BaseTest {
    @Resource
    ...
    public void test() {
        ...
        searchCriteria.setFilter("{'$like':{'email':'$abc$'}}");
        searchCriteria.setEntityName("User");
        searchQueryBuilder.doBuild(searchCriteria);
        Assert.assertEquals("FROM User entity WHERE entity.email LIKE :$1", searchCriteria.getListQuery());
    }
}